(function () {
  ...
  for (var i = 0; i < 8502; i++) {
    ...
    data.push({
      ...
      location: i % 2 ? 'A01-01-01' : '--', exception: '-- / --',
      voidBy: '--', voidAt: '--', created: created,
      ...
      weight: i % 4 === 0 ? '2.4567' : '--', processFee: '--', cost: '--', inboundQty: i % 6 === 0 ? 5 : 1,
      shelfQty: i % 6 === 0 ? 5 : 1, badQty: 0, batch: batch, productBatch: i % 4 === 0 ? '2026091801' : '--',
      ...
      logs: [{ attribute: '创建入库任务', description: '创建入库任务，等待签收', operator: 'HF', time: created }, { attribute: '签收', description: '商品签收完成', operator: i % 3 ? 'XWD' : 'HF', time: created }, { attribute: '入库完成', description: '商品入库完成', operator: 'XWD', time: created }]
    });
  }
  ...
  controls.log = ERPComponents.createOperationLog({ title: '操作日志', operationAttributes: ['创建入库任务', '签收', '质检', '贴标', '加工', '上架', '入库完成'], operators: people.map(function (value) { return { value: value, label: value, enabled: true }; }), records: [] });
  ...
  function renderRow(row, index) {
    ...
    return '<tr class="' + (selected ? 'selected' : '') + '"><td class="center"><div class="row-selector"><span>' + (index + 1) + '</span><button class="expand" data-expand="' + row.id + '" type="button">' + (expanded ? '−' : '+') + '</button><input type="checkbox" class="row-check" data-id="' + row.id + '" ' + (selected ? 'checked' : '') + '></div></td>' +
      ...
      '<td><div class="inbound-operations"><button type="button" data-log="' + row.id + '">操作日志</button></div></td></tr>' +
  ...
  $('#exportBtn').addEventListener('click', function () { var rows = state.selected.size ? state.filtered.filter(function (row) { return state.selected.has(row.id); }) : state.filtered; var name = '历史入库日志-' + Date.now() + '.csv'; downloadCsv(name, rows); exportTasks.unshift({ name: name, operator: account, time: formatTime(new Date()), status: '成功' }); $('#exportWrap').classList.remove('open'); toast('导出成功'); });
