(function () {
  ...
  var exportTasks = [];
  var controls = {};
  var state = { page: 1, size: 10, filtered: [], selected: new Set(), expanded: new Set(), shipType: '全部', orderType: '全部', pickupStatus: '全部', custom: '全部', deduct: '全部', pickupMode: 'modify' };
  ...
  $('#exportBtn').addEventListener('click', function () { var rows = state.selected.size ? selectedRows() : state.filtered; csv(rows); exportTasks.unshift({ name: '历史订单出库日志导出', operator: 'XWD', time: formatTime(new Date()), status: '成功' }); $('#exportWrap').classList.remove('open'); toast('导出成功'); });
